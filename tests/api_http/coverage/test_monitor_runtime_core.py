...
def _authenticate_monitor(
    ...
    monitor_http_client.cookies.update(web_client.cookies)
...
def test_monitor_permission_is_developer_only(
    ...
    web_client = authenticated_client_for(
        {"guest": guest_user, "student": student_user, "teacher": teacher_user}[role]
    )
    monitor_http_client.cookies.update(web_client.cookies)
    response = monitor_http_client.get(
        "/api/v1/auth/session",
        headers=same_origin_headers(monitor_base_url),
    )
...
def test_monitor_observability_and_sandbox_routes_are_real_and_bounded(
    ...
    session = _authenticate_monitor(monitor_http_client, web_client, monitor_base_url)
    ...
    for path in (
        ...
        "/api/v1/observability/sessions?days=1&limit=1",
