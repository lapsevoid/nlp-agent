...
function cacheHitRate(tokens: Record<string, number>) {
  const inputTokens = Math.max(0, Number(tokens.input_tokens ?? 0));
  if (!inputTokens) return null;
  const cachedInputTokens = Math.max(0, Number(tokens.cached_input_tokens ?? tokens.cached_tokens ?? 0));
  return Math.min(cachedInputTokens, inputTokens) / inputTokens;
}
...
export function MonitorUsagePage({ data, usage, systemUsage }: { data: Overview; usage: UsageRow[]; systemUsage: SystemUsageSnapshot | null }) {
  ...
  const measuredCacheHitRate = systemUsage?.cache_hit_rate ?? cacheHitRate(tokenValues);
  ...
  return <div className="mon-page mon-usage-page">
    ...
    <section className="mon-usage-summary" aria-label="用量中心摘要"><div className="mon-usage-summary-lead"><span>全用户总 Token</span><strong>{fmt(tokenValues.total_tokens)}</strong><small>{fmt(systemUsage?.events ?? data.requests)} 次事件 · {fmt(data.active_users)} 位活跃用户</small></div><div><span>输入</span><strong>{fmt(tokenValues.input_tokens)}</strong><small>含缓存读取 {fmt(tokenValues.cached_input_tokens)}</small></div><div><span>KV Cache 命中率</span><strong>{percentage(measuredCacheHitRate)}</strong><small>{fmt(tokenValues.cached_input_tokens)} / {fmt(tokenValues.input_tokens)} 输入 Token</small></div><div><span>输出</span><strong>{fmt(tokenValues.output_tokens)}</strong><small>含推理 {fmt(tokenValues.reasoning_output_tokens)}</small></div><div><span>计价状态</span><strong className={systemUsage?.credits_complete === false ? "danger-text" : "success-text"}>{systemUsage ? (systemUsage.credits_complete ? "完整" : "部分") : "—"}</strong><small>{fmt(systemUsage?.priced_events)} 已计价 · {fmt(systemUsage?.unpriced_events)} 待补</small></div></section>
