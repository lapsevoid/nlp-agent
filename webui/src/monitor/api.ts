...
export interface SystemUsageSnapshot { scope: "system"; period_days: number; from: string; to: string; granularity: string; events: number; priced_events: number; unpriced_events: number; credits_complete: boolean; credit_status: string; credits_micro: number | null; priced_credits_micro: number; tokens: Record<string, number>; cache_hit_rate?: number | null; cache_input_tokens?: number | null; cache_cached_input_tokens?: number | null; cache_measured_events?: number; cache_unmeasured_events?: number; worker_cache_hit_rate?: number | null; worker_cache_input_tokens?: number | null; worker_cache_cached_input_tokens?: number | null; worker_cache_measured_events?: number; worker_cache_unmeasured_events?: number; breakdown: SystemUsageBreakdown[]; users: SystemUsageDimension[]; workspaces: SystemUsageDimension[]; providers: SystemUsageDimension[]; purposes: SystemUsageDimension[]; models: SystemUsageDimension[]; role_models?: SystemUsageDimension[]; catalog?: SystemUsageCatalog; window_minutes?: number; bucket_minutes?: number; }
